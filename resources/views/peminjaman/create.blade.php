        ...
        <form action="{{ 'store' }}" method="post">
            ...
            <div class="mb-3">
                ...
                <select name="status" id="status" class="form-control">
                    <option value="masih di pinjam" {{ old('status') === 'masih di pinjam' ? 'selected' : '' }}>masih di pinjam</option>
                    <option value="sudah di kembalikan" {{ old('status') === 'sudah di kembalikan' ? 'selected' : '' }}>sudah di kembalikan</option>
                </select>                
                @error('status')
                <p class="text-danger">{{ $message }}</p>
                @enderror
            ...
            <button type="submit" class="btn btn-primary -vh100">Save</button>
        </form>
    </main>
@endsection
